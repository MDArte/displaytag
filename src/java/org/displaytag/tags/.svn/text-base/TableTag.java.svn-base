...
public class TableTag extends HtmlTableTag
{
    ...
    /**
     * name for the group attribute
     */
    public static final String PAGE_GROUP = "group"; //$NON-NLS-1$
    
    /**
     * nome da tabela que foi passada como parametro
     */
    ...
    /**
     * pagina que ta na session
     */
    private NumberedPage numberedPage;
     ...
     * Request uri Action
     */
    private String requestUriAction;
    ...
    /**
     * nome atual da table 
     * importante para saber de que table veio quando vier do servidor
     */
     ...
     */
    private int defaultSortedColumn = 0;
    ...
    /**
     * base href used for prev, first and next link - set in initParameters().
     */
    private Href baseHrefAction;

    /**
     * nome passado como parametro na url
     */
    ...
    /**
     * setter for the "requestURIAction" attribute. Context path is automatically added to path starting with "/".
     * @param value base URI for creating links
     */
    public void setRequestURIAction(String value)
    {
        this.requestUriAction = value;
    }    
    ...
    /**
     * Returns the base href action with parameters. This is the instance used for prev, first and next links, need to be cloned before being
     * modified.
     * @return base Href with parameters
     */    
    protected Href getBaseHrefAction()
    {
        return this.baseHrefAction;
    }
    ...
    public int doStartTag() throws JspException
    {
        ...
        //String currentTableName = pageContext.getRequest().getParameter(encodeParameter(TableTagParameters.PARAMETER_TABLE));                
        
        this.currentGroup = (Integer) pageContext.getRequest().getAttribute(PAGE_GROUP);	  
        this.currentTableName = (String) pageContext.getRequest().getAttribute(TABLE_NAME);
    ...
    private void initParameters() throws ObjectLookupException, FactoryInstantiationException
    {
        ...
        paramTableName = pageContext.getRequest().getParameter(encodeParameter(TableTagParameters.PARAMETER_TABLE));
        //boolean paginacao = false;
        Integer pageNumberParameter = null;
        Integer pageGroupParamenter = null;
    	initHref(requestHelper);            
    	initHrefAction(requestHelper);       
    	
    	boolean paginacao = verificaPaginacao();

    	
        //if(currentTableName == null || currentTableName.equals(getTableName())) 
    	if(paginacao)
        {
        	pageNumberParameter = requestHelper.getIntParameter(encodeParameter(TableTagParameters.PARAMETER_PAGE));
        	pageGroupParamenter = requestHelper.getIntParameter(encodeParameter(TableTagParameters.PARAMETER_GROUP));
        }else{
        	Object obj = this.getPageContext().getSession().getAttribute(tableName);
        	if(obj != null)
        		numberedPage = (NumberedPage) obj;
        	else
        		numberedPage = new NumberedPage(1, true, 1);
    		pageNumberParameter = new Integer(numberedPage.getNumber());       
    		//pageGroupParamenter = new Integer(numberedPage.getGroup());
    		this.currentGroup = new Integer(numberedPage.getGroup());
        }
...
//      this.pageNumber = (pageNumberParameter == null) ? 1 : pageNumberParameter.intValue();
        ...
        if (log.isDebugEnabled())
        {
            log.debug("GROUPSIZE:[" + String.valueOf(groupSize)  + "]");
            log.debug("PAGINA:[" + String.valueOf(pagina) + "]");            
            log.debug("PAGEGROUPPARAMETER:[" + String.valueOf(pageGroupParamenter) + "]");
            log.debug("PAGENUMBERPARAMETER:[" + String.valueOf(pageNumberParameter) + "]");
            log.debug("CURRENTGROUP:[" + String.valueOf(this.currentGroup) + "]");            
        }
        // Remover (*)
        ...
        if (log.isDebugEnabled())
        {
            log.debug("PAGENUMBER:[" + String.valueOf(this.pageNumber) + "]");
            log.debug("CURRENTGROUP:[" + String.valueOf(this.currentGroup) + "]");            
        }
        // Remover (*)
            ...
            ))
        {
            ...
            if (this.pagesize > 0)
            {
                ...
                if (log.isDebugEnabled())
                {
                    log.debug("FULLSIZE:[" + String.valueOf(fullSize)  + "]");                
                }
                // Remover (*)
    ...
    private boolean verificaPaginacao() {    	
    	boolean paginacao = true;
    	if(paramTableName == null){//veio do servidor
    		
    		if(currentGroup != null && currentTableName != null && !currentTableName.equals(getTableName())){//veio de mudanca de grupo
    			paginacao = false;
    		}
    	
    	}else{//veio de navegacao entre paginas, nao entre grupos
    		if(!paramTableName.equals(getTableName())){
    			paginacao = false;
    		}
    	}
    	return paginacao;
    ...
    protected boolean isIncludedRow()
    {
        ...
        if (log.isDebugEnabled())
        {
            log.debug("INCLUDE:[" + String.valueOf(((Range) filteredRows).containsLong(this.rowNumber))  + "]");
            log.debug("ROWNUMBER:[" + String.valueOf(rowNumber) + "]");            
            log.debug("ROWNUMBER:[" + ((Range) filteredRows).toString() + "]");            
        }
        // Remover (*)
    ...
    /**
     * init the href object used to generate all the links for next, prev and first pagination.
     * @param requestHelper request helper used to extract the base Href
     */    
    protected void initHrefAction(RequestHelper requestHelper){
    	Href normalHref = requestHelper.getHref();
        if (this.requestUriAction != null)
        {
            // if user has added a requestURI create a new href
            String fullURI = requestUriAction;

            // call encodeURL to preserve session id when cookies are disabled
            fullURI = ((HttpServletResponse) this.pageContext.getResponse()).encodeURL(fullURI);
            this.baseHrefAction = new Href(fullURI);
        ...
        {
            // simply copy href
        	String hrefTeste = normalHref.toString() + ".do";
            this.baseHrefAction = new Href(hrefTeste);
        }    	
    }
    /**
    ...
    public int doEndTag() throws JspException
    {
        ...
        /*
         * Condição que decide se os elementos da tabela serao ordenados ou nao
         * 
         * Se ocorre paginacao no hibernate e for o primeiro grupo de paginacao
         *          Nao ordena a lista independente do criterio do usuario
         * Caso contrario
         * 			Criteio de ordenacao definido pelo usuario no modelo
         * 
         */
        
        List groupHibernate  = (List) this.list ;
        ...
        if (MediaTypeEnum.HTML.equals(this.currentMediaType)
            && (previousMediaType == null || MediaTypeEnum.HTML.equals(previousMediaType)) || !getTableName().equals(paramTableName))
        {
            writeHTMLData();
        }
    ...
    private void writeSearchResultAndNavigation()
    {
        if (this.pagesize != 0 && this.listHelper != null)
        {
            ...
            Href navigationHrefAction = new Href(this.baseHrefAction);

            write(this.listHelper.getSearchResultsSummary());
            Object obj = getPageContext().getSession().getAttribute(getTableName());
            if(obj != null)
            	numberedPage = (NumberedPage) obj;
            write(this.listHelper.getPageNavigationBar(
                navigationHref, navigationHrefAction,
                encodeParameter(TableTagParameters.PARAMETER_PAGE), encodeParameter(TableTagParameters.PARAMETER_GROUP), encodeParameter(TableTagParameters.PARAMETER_TABLE), getTableName(), pageContext.getSession()));
    ...
    private void writeExportLinks()
    {
        ...
        while (iterator.hasNext())
        {
            ...
            if (this.properties.getAddExport(currentExportType))
            {
                ...
                exportHref.addParameter(encodeParameter(TableTagParameters.PARAMETER_TABLE), getTableName());
